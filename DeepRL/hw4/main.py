...
from DeepRL.hw4 import logz
from DeepRL.learn import variables
...
class NnValueFunction(object):
    ...
    def init_network(self):
        ...
        self.X = tf.placeholder(shape=[None, self.ob_dim], name="vf/X", dtype=tf.float32)
        self.y = tf.placeholder(shape=[None], name="vf/y", dtype=tf.float32)

        network = layers.fully_connected(self.X, 128, activation="relu", name="vf/fc1")
        ...
        self.output = layers.fully_connected(network, 1, name="vf/output")
        self.loss = objectives.mean_square(self.output, self.y)
        self.train_op = optimizers.Momentum(self.stepsize).minimize(self.loss)
    ...
    def fit(self, X, y):
        ...
        for i in range(self.n_epochs):
            ...
            batches = make_batches(len(y), 128)
            for j, (start, end) in enumerate(batches):
                self.session.run(self.train_op, feed_dict={self.X: X[start:end], self.y: y[start:end]})

    def predict(self, X):
        cfg.is_training(False, session=self.session)
...
def main_cartpole(n_iter=100, gamma=1.0, min_timesteps_per_batch=1000, stepsize=1e-2, animate=True, logdir=None):
    ...
    vf = NnValueFunction(ob_dim, 10, 0.0001)
...
def main_pendulum(logdir, seed, n_iter, gamma, min_timesteps_per_batch, initial_stepsize,
                  desired_kl, vf_type, vf_params, animate=True):
    ...
    sy_mean_na = dense(sy_h1, ac_dim, "final", weight_init=normc_initializer(0.1))
    logstd_a = variables.variable(shape=[ac_dim], initializer="zeros", name="logstdev")
    # we use a small initialization for the last layer, so the initial policy has maximal entropy
    sy_oldmean_na = tf.placeholder(shape=[None, ac_dim], name='oldmean', dtype=tf.float32)
    sy_sampled_ac = tf.random_normal(shape=tf.shape(sy_mean_na), mean=sy_mean_na, stddev=tf.exp(logstd_a))[0]
    dist = tf.contrib.distributions.Normal(mu=sy_mean_na, sigma=tf.exp(logstd_a))
    sy_logprob_n = tf.reduce_prod(dist.log_prob(sy_ac_n), axis=1)

    # The following quantities are just used for computing KL and entropy, JUST FOR DIAGNOSTIC PURPOSES >>>>
    sy_n = tf.shape(sy_ob_no)[0]
    s_a_squared = tf.square(tf.exp(logstd_a))
    s_b_squared = tf.square(tf.exp(logstd_a))
    ratio = s_a_squared / s_b_squared
    sy_kl = tf.reduce_sum(tf.square(sy_mean_na - sy_oldmean_na) / (2 * s_b_squared) + 0.5 * (ratio - 1 - tf.log(ratio))) / tf.to_float(sy_n)
    sy_ent = tf.reduce_sum(dist.entropy()) / tf.to_float(sy_n)
    # <<<<<<<<<<<<<
    ...
    for i in range(n_iter):
        ...
        timesteps_this_batch = 0
        paths = []
        animate_this_episode = ((i % 20 == 0) and (i > 100) and animate)
        ob = env.reset()
        terminated = False
        obs, acs, rewards = [], [], []
        while True:
        ...
        # Policy update
        _, oldmean_na = sess.run(
            [update_op, sy_mean_na],
            feed_dict={
                sy_ob_no: ob_no, sy_ac_n: ac_n,
                sy_adv_n: standardized_adv_n,
                sy_stepsize: stepsize
...
if __name__ == "__main__":
    if 1:
        main_pendulum(logdir='/tmp/ref/nnvf-kl2e-3-seed0', seed=0, desired_kl=2e-3, vf_type='nn', vf_params=dict(n_epochs=10, stepsize=1e-3),
                      gamma=0.97, animate=False, min_timesteps_per_batch=2500, n_iter=150, initial_stepsize=1e-3)
    if 0:
